...
async function _connectivtySocks(jwt, pgCredentials, credentials) {
  let sLocationBase64 = Buffer.from(pgCredentials.connectivityLocationId).toString('base64')
  ...
  const options = {
    ...
    destination: {
      ...
      port: parseInt(pgCredentials.port),
    },
    timeout: 30000,
  }
...
async function _connectivityStream(credentials) {
  xsenv.loadEnv()
  const connectivityCredentials = xsenv.cfServiceCredentials('connectivity')

  if (!connectivityCredentials) {
    log('ERROR: No connectivity credentials provided (local: check env, SAP BTP: check binding)')
  }

  try {
    let jwt = await _connectivityToken(connectivityCredentials)
    let socket = await _connectivtySocks(jwt, credentials, connectivityCredentials)
    return { stream: socket, host: connectivityCredentials.onpremise_proxy_host }
  } catch (err) {
    log(err)
  }
}
 ...
 */
async function connectivityStream(credentials) {
  return await _connectivityStream(credentials)
}
