...
module.exports = class PostgresDatabase extends cds.DatabaseService {
  constructor(...args) {
    ...
    if (this.options.credentials && !!this.options.credentials.connectivityEnabled) {
      const { connectivityStream } = require('./utils/connectivity')
      connectivityStream(this.options.credentials).then(
        function (options) {
          if (options.stream.constructor !== require('net').Socket) {
            throw new Error(`Stream is not of type socket: ${options.stream.constructor}`)
          }
          this.options.credentials.stream = options.stream
          this.options.credentials.host = options.host // Virtualhost would otherwise result in "Error: getaddrinfo ENOTFOUND"
          this.options.credentials.port = 20004
          this.createPgPool()
        }.bind(this)
      )
    } else {
