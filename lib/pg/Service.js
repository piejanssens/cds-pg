  ...
  constructor(...args) {
    ...
    if (this.options.credentials && !!this.options.credentials.connectivityEnabled) {
      ...
      connectivityStream(this.options.credentials).then(
        function (socket) {
          this.options.credentials.stream = socket
          this.options.credentials.host = 'connectivityproxy.internal.cf.eu10.hana.ondemand.com' // Virtualhost would otherwise result in "Error: getaddrinfo ENOTFOUND"
          this.createPgPool()
        }.bind(this)
      )
    } else {
