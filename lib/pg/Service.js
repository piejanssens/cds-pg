...
module.exports = class PostgresDatabase extends cds.DatabaseService {
  constructor(...args) {
    ...
    if (this.options.credentials && !!this.options.credentials.connectivityEnabled) {
      ...
      connectivityStream(this.options.credentials).then((options) => {
        if (options.stream.constructor !== require('net').Socket) {
          throw new Error(`Expecting net.Socket, instead got ${options.stream.constructor}`)
        }
        delete this.options.credentials.host
        delete this.options.credentials.port
        this.options.credentials.stream = options.stream
        this.options.credentials.log = console.log
        this.createPgPool()
      })
    } else {
      this.createPgPool()
